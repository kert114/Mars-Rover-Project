...
//F12.6
//x:y:A:@TD:@TA
std::string turn_to_object (float angle_from_drive , float angle_from_vision){
  ...
  return "T" + std::to_string(angle_to_drive); // this needs to return the function so that it turns. Then in the main script, call another function that doesn't do this. 
...
std::string obstacle_avoidance (float distance_to_alien,float angle_from_vision, float angle_from_drive,  bool alien_detected, int xto, int yto, float xd, float yd , bool dest_angle, bool dest_distance, bool has_vision_sent){
  ...
  if(alien_detected && !(dest_angle && dest_distance)){
    ...
    if(distance_to_alien<30){
      ...
      else if (has_vision_sent){
        command_to_drive = "T" + std::to_string(current_angle_saved);// return back to original angle
        if(dest_angle){
