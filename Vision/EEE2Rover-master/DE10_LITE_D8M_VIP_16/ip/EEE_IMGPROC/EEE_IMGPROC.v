...
wire red_detect;
assign red_detect = ((hue>0 && hue <20) || (hue>356 && hue <360)) && (saturation>50) && (value>60 );//&& value<75);
wire teal_detect;
assign teal_detect = (hue>100 && hue <155) && (saturation>22 && saturation<74) && (value>6 && value<47);
wire fuchsia_detect;
assign fuchsia_detect = (hue>320 && hue <=360) && (saturation>50) && (value>=35);
wire orange_detect;
assign orange_detect = (hue>=30 && hue<=50) && (saturation>55) && (value>=75);
...
//	end
	...
	if (orange_detect & in_valid) begin	//Update bounds when the pixel is fuchsia
		if (x < x_min) x_min <= x;
		if (x > x_max) x_max <= x;
		if (y < y_min) y_min <= y;
		y_max <= y;
...
always@(*) begin	//Write words to FIFO as state machine advances
	case(msg_state)
		...
		2'b01 && orange_detect: begin
			msg_buf_in = `ORANGE_BOX_MSG_ID;	//Message ID
			msg_buf_wr = 1'b1;
		end
		2'b10: begin
